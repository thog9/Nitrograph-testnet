...
LOYALTIES_RULES_URL = f"{COMMUNITY_URL}/api/loyalties/rules"
...
TASK = {
    "taskId": "daily_streak",
    "url": LOYALTIES_RULES_URL,
    "points": 20,
    "method": "POST",
    "payload": {"ruleIds": ["66384e13-4c6b-4aa4-ae2f-6a10a650aaa9"]}
}

# Bilingual vocabulary
LANG = {
    'vi': {
        'title': 'TỰ ĐỘNG HOÀN THÀNH NHIỆM VỤ DAILY CHECKIN - NITROGRAPH',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        ...
        'completing_task': 'Đang hoàn thành nhiệm vụ daily_claim...',
        'task_success': '✅ Hoàn thành nhiệm vụ daily_claim (+{points} điểm)!',
        'task_failure': '❌ Thất bại khi hoàn thành nhiệm vụ daily_claim: {error}',
        'pausing': 'Tạm dừng',
        'seconds': 'giây',
        'completed': '✔ HOÀN THÀNH: {successful}/{total} NHIỆM VỤ DAILY CHECKIN',
        'error': 'Lỗi',
        'found_wallets': 'Thông tin: Tìm thấy {count} ví'
    },
    'en': {
        'title': 'AUTOMATIC DAILY CHECKIN TASK COMPLETION - NITROGRAPH',
        'info': 'Information',
        'found': 'Found',
        ...
        'completing_task': 'Completing daily_claim task...',
        'task_success': '✅ Completed daily_claim task (+{points} points)!',
        'task_failure': '❌ Failed to complete daily_claim task: {error}',
        'pausing': 'Pausing',
        'seconds': 'seconds',
        'completed': '✔ COMPLETED: {successful}/{total} DAILY CHECKIN TASKS',
        'error': 'Error',
            ...
            async with aiohttp.ClientSession(connector=connector, timeout=aiohttp.ClientTimeout(total=CONFIG['TIMEOUT'])) as session:
                if task['method'] == "POST":
                    async with session.post(task['url'], headers=headers, json=task['payload']) as response:
                        if response.status == 200:
                            print(f"{Fore.GREEN} ✔ {LANG[language]['task_success'].format(points=task['points'])}{Style.RESET_ALL}")
                            return True
                        error_text = await response.text()
...
async def process_wallet(index: int, account: LocalAccount, language: str = 'vi', proxies: List[str] = None) -> bool:
    ...
    if not await complete_task(token, TASK, language, proxy):
        print(f"{Fore.RED} ✖ Bỏ qua ví {index + 1} do thất bại khi hoàn thành nhiệm vụ daily_streak{Style.RESET_ALL}")
        return False

    return True

async def run_dailyxp(language: str = 'vi'):
    print()
