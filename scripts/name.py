...
UPDATE_NAME_URL = f"{COMMUNITY_URL}/api/users/me/metadata"
...
LANG = {
    'vi': {
        'title': 'TỰ ĐỘNG CẬP NHẬT TÊN - NITROGRAPH',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        ...
        'updating_name': 'Đang cập nhật tên...',
        'name_success': '✅ Cập nhật tên thành công!',
        'name_failure': '❌ Cập nhật tên thất bại: {error}',
        'pausing': 'Tạm dừng',
        'seconds': 'giây',
        'completed': '✔ HOÀN THÀNH: {successful}/{total} CẬP NHẬT TÊN',
        'error': 'Lỗi',
        'input_name': 'Nhập tên muốn cập nhật (VD: Thog): ',
        'found_wallets': 'Thông tin: Tìm thấy {count} ví'
    },
    'en': {
        'title': 'AUTOMATIC NAME UPDATE - NITROGRAPH',
        'info': 'Information',
        'found': 'Found',
        ...
        'name_failure': '❌ Name update failed: {error}',
        'pausing': 'Pausing',
        'seconds': 'seconds',
        'completed': '✔ COMPLETED: {successful}/{total} NAME UPDATES',
        'error': 'Error',
        'input_name': 'Enter the name to update (e.g., Thog): ',
...
async def update_name(token: str, address: str, display_name: str, language: str = 'vi', proxy: str = None) -> bool:
    print(f"{Fore.CYAN} > {LANG[language]['updating_name']}{Style.RESET_ALL}")
    ...
    for attempt in range(CONFIG['RETRY_ATTEMPTS']):
        try:
            connector = ProxyConnector.from_url(proxy) if proxy else None
            async with aiohttp.ClientSession(connector=connector, timeout=aiohttp.ClientTimeout(total=CONFIG['TIMEOUT'])) as session:
                async with session.post(UPDATE_NAME_URL, headers=headers, json=payload) as response:
                    if response.status == 200:
                        print(f"{Fore.GREEN} ✔ {LANG[language]['name_success']}{Style.RESET_ALL}")
                        print(f"{Fore.YELLOW}    - Name: {display_name}{Style.RESET_ALL}")
                        print(f"{Fore.YELLOW}    - Địa chỉ ví: {address}{Style.RESET_ALL}")
                        return True
                    error_text = await response.text()
                    print(f"{Fore.RED} ✖ {LANG[language]['name_failure'].format(error=f'HTTP {response.status}: {error_text}')}{Style.RESET_ALL}")
                    return False
        except Exception as e:
            if attempt < CONFIG['RETRY_ATTEMPTS'] - 1:
                await asyncio.sleep(CONFIG['RETRY_DELAY'])
                continue
            print(f"{Fore.RED} ✖ {LANG[language]['name_failure'].format(error=str(e))}{Style.RESET_ALL}")
            return False
    return False

async def process_wallet(index: int, account: LocalAccount, display_name: str, language: str = 'vi', proxies: List[str] = None) -> bool:
    ...
    # Update name
    if not await update_name(token, address, display_name, language, proxy):
        print(f"{Fore.RED} ✖ Bỏ qua ví {index + 1} do cập nhật tên thất bại{Style.RESET_ALL}")
        return False

    return True

async def run_update_name(language: str = 'vi'):
    print()
    ...
    # Get user input for display name
    display_name = input(f"{Fore.CYAN}{LANG[language]['input_name']}{Style.RESET_ALL}").strip()
    if not display_name:
        print(f"{Fore.RED} ✖ Tên hiển thị không được để trống{Style.RESET_ALL}")
        sys.exit(1)
    ...
    async def sem_process_wallet(index: int, account: LocalAccount):
        ...
        async with semaphore:
            try:
                success = await process_wallet(index, account, display_name, language, proxies)
                total_wallets += 1
                if success:
...
if __name__ == "__main__":
    asyncio.run(run_update_name('vi'))
