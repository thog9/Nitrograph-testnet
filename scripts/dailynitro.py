...
CREDITS_CLAIM_URL = f"{API_BASE_URL}/api/credits/claim"
...
LANG = {
    'vi': {
        'title': 'TỰ ĐỘNG HOÀN THÀNH NHIỆM VỤ DAILY STREAK - NITROGRAPH',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        ...
        'completing_task': 'Đang hoàn thành nhiệm vụ daily_streak...',
        'task_success': '✅ Hoàn thành nhiệm vụ daily_streak (+{points} điểm)!',
        'task_failure': '❌ Thất bại khi hoàn thành nhiệm vụ daily_streak: {error}',
        'pausing': 'Tạm dừng',
        'seconds': 'giây',
        'completed': '✔ HOÀN THÀNH: {successful}/{total} NHIỆM VỤ DAILY STREAK',
        'error': 'Lỗi',
        'found_wallets': 'Thông tin: Tìm thấy {count} ví'
    },
    'en': {
        'title': 'AUTOMATIC DAILY STREAK TASK COMPLETION - NITROGRAPH',
        'info': 'Information',
        'found': 'Found',
        ...
        'completing_task': 'Completing daily_streak task...',
        'task_success': '✅ Completed daily_streak task (+{points} points)!',
        'task_failure': '❌ Failed to complete daily_streak task: {error}',
        'pausing': 'Pausing',
        'seconds': 'seconds',
        'completed': '✔ COMPLETED: {successful}/{total} DAILY STREAK TASKS',
        'error': 'Error',
...
async def process_wallet(index: int, account: LocalAccount, language: str = 'vi', proxies: List[str] = None) -> bool:
    ...
    if not await complete_task(token, TASK, language, proxy):
        print(f"{Fore.RED} ✖ Bỏ qua ví {index + 1} do thất bại khi hoàn thành nhiệm vụ daily_claim{Style.RESET_ALL}")
        return False

    return True

async def run_dailynitro(language: str = 'vi'):
    print()
